...
import { CreatorCollabRequest } from "../models/CreatorRequestCollab.model.js";
...
export const requestCollabAsCreator = async (req, res) => {
  try {
    ...
    const existing = await CreatorCollabRequest.findOne({ sponsorCampaignId, creatorId: creator._id });
    if (existing) {
      return res.status(400).json({ message: "A collab request for this campaign already exists" });
    }

    const collab = await CreatorCollabRequest.create({ sponsorCampaignId, creatorId: creator._id });
